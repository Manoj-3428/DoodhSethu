...
class AutoSyncManager(private val context: Context) {
    ...
    /**
     * Update all farmer profiles after data restoration
     */
    private suspend fun updateAllFarmerProfiles() = withContext(Dispatchers.IO) {
        try {
            Log.d("AutoSyncManager", "Updating all farmer profiles after data restoration")
            ...
            Log.d("AutoSyncManager", "All farmer profiles updated successfully")
        } catch (e: Exception) {
            Log.e("AutoSyncManager", "Error updating farmer profiles: ${e.message}")
        }
    }
    ...
    suspend fun preloadAllScreenDataWithProgress(onProgress: (Int, String) -> Unit) {
        try {
            ...
            try {
                ...
                onProgress(75, "Duplicate entries cleaned successfully")
            } catch (e: Exception) {
                Log.e("AutoSyncManager", "❌ Failed to cleanup duplicates: ${e.message}")
                onProgress(75, "Duplicate cleanup with errors")
            }
